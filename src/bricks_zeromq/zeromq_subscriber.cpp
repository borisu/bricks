...
zeromq_subscriber_t::init(cb_queue_t* queue, topic_cb_t msg_cb, const xtree_t* options)
{
	...
	try
	{
		set_sockopt(options, "/bricks/zmq/subscriber/methods/init/socket", subscriber);

		name = get<string>(options->get_property_value("/bricks/zmq/subscriber", "name").value());

		url = get<string>(options->get_property_value("/bricks/zmq/subscriber/methods/init/socket", "url").value());

		auto is_server = get<bool>(options->get_property_value("/bricks/zmq/subscriber/methods/init/socket", "is_server").value());
...
void 
zeromq_subscriber_t::set_meta_cb(meta_cb_t meta_cb)
{
	this->meta_cb = meta_cb;
}

void
zeromq_subscriber_t::do_destroy()
{
	SYNCHRONIZED(mtx);
